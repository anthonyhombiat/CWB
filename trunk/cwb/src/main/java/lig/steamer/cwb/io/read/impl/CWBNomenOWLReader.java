package lig.steamer.cwb.io.read.impl;

import java.io.File;
import java.io.InputStream;
import java.util.Set;
import java.util.logging.Level;
import java.util.logging.Logger;

import lig.steamer.cwb.io.read.CWBNomenReader;
import lig.steamer.cwb.io.read.impl.exception.CWBNomenReaderException;
import lig.steamer.cwb.model.CWBConcept;
import lig.steamer.cwb.model.CWBDataModelNomen;

import org.semanticweb.owlapi.apibinding.OWLManager;
import org.semanticweb.owlapi.model.OWLClass;
import org.semanticweb.owlapi.model.OWLClassExpression;
import org.semanticweb.owlapi.model.OWLOntology;
import org.semanticweb.owlapi.model.OWLOntologyCreationException;
import org.semanticweb.owlapi.model.OWLOntologyManager;

public class CWBNomenOWLReader implements CWBNomenReader {

	private static Logger LOGGER = Logger.getLogger(CWBNomenOWLReader.class
			.getName());

	private OWLOntologyManager manager;
	private OWL2CWB owl2cwb;

	public CWBNomenOWLReader() {
		manager = OWLManager.createOWLOntologyManager();
		owl2cwb = new OWL2CWB(manager.getOWLDataFactory());
	}

	@Override
	public CWBDataModelNomen read(File file) throws CWBNomenReaderException {

		try {
			OWLOntology ontology = manager
					.loadOntologyFromOntologyDocument(file);

			return read(ontology);

		} catch (OWLOntologyCreationException e) {
			throw new CWBNomenReaderException(e);
		}

	}

	@Override
	public CWBDataModelNomen read(InputStream inputStream)
			throws CWBNomenReaderException {

		try {
			OWLOntology ontology = manager
					.loadOntologyFromOntologyDocument(inputStream);

			return read(ontology);

		} catch (OWLOntologyCreationException e) {
			throw new CWBNomenReaderException(e);
		}

	}
	
	@Override
	public CWBDataModelNomen read(String filename)
			throws CWBNomenReaderException {

		return read(new File(filename));

	}

	private CWBDataModelNomen read(OWLOntology ontology) {
		LOGGER.log(Level.INFO, "Reading nomenclature " + ontology.getOntologyID().getOntologyIRI() + "...");

		CWBDataModelNomen nomen = populateDataModel(new CWBDataModelNomen(
				ontology.getOntologyID().getOntologyIRI()), ontology,
				owl2cwb.findRootClasses(ontology), null);

		LOGGER.log(Level.INFO, "Nomenclature read (" + nomen.getConcepts().size() + " concepts).");

		return nomen;
	}

	private CWBDataModelNomen populateDataModel(CWBDataModelNomen dataModel,
			OWLOntology ontology, Set<? extends OWLClassExpression> classes,
			CWBConcept parent) {

		for (OWLClassExpression classExpression : classes) {

			OWLClass clazz = classExpression.asOWLClass();

			CWBConcept concept = owl2cwb.getCWBConceptFromOwlClass(clazz, ontology);

			if (!dataModel.getConcepts().contains(concept)) {

				if (parent != null) {
					concept.setParent(parent);
				}

				dataModel.addConcept(concept);

				LOGGER.log(Level.FINE, "Concept " + concept.getFragment()
						+ " added to data model.");

				populateDataModel(dataModel, ontology,
						clazz.getSubClasses(ontology), concept);

			}
		}

		return dataModel;
	}

}
